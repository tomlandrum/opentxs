    ...
    const OTTransaction& inboxTransaction)
{
    ...
    switch (originalItem.GetType()) {
        case itemType::depositCheque: {
            ...
            // TODO remove legacy pre-workflow code
            {
                auto nymfile = mutable_Nymfile("");
                auto message = nymfile.It().GetOutpaymentsByTransNum(number);

                if (message) {
                    nymfile.It().RemoveOutpaymentsByTransNum(number);
                }
            }
        } break;
